    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getPaymentRecord.fulfilled, (state, action) => {
         ...
         builder.addCase(cancelCourseBundle.fulfilled, (state, action) => {
              console.log("Subscription status in RazorpaySlice.fulfilled:", action.payload.status);
        })

        .addMatcher(
            (action) => action.type.endsWith('/rejected'),
            (state, action) => {
                // Common place to handle global toast errors from thunks
                // toast.error(action.error.message || 'Operation failed');
            }
        );
    }
})
